...
def inititalize_graph(st, req, emReq):
            ...
            for x2 in (-1,0,1):
                for y2 in (-1,0,1):
                    ...
                    if (x1, y1) in obstacles or (x1 + x2, y1 + y2) in obstacles:
                        ...
                        graph[(x1 + x2, y1 + y2)][(x1, y1)] = math.inf
                        continue
                    graph[(x1,y1)][(x1 + x2, y1 + y2)] = graph[(x1 + x2, y1 + y2)][(x1, y1)] = math.sqrt((x2)**2 + (y2)**2)
    ...
    for x in range(V):
        if x != V-1:
            grid, = plt.plot([i for i in range(0, V)], [x + 0.5] * V, c= 'black')
        for y in range(V):
            if y != V-1:
                plt.plot([y+0.5] * V, [i for i in range(0, V)], c= 'black')
            if (x,y) == (3,4):
                plt.scatter(x, y, c="green", marker= '*', label= 'hub')
                continue
            if (x,y) in obstacles:
    ...
    # plt.show()
    # plt.legend()
    emSt.set_label('emergency requests')
    stations.set_label('active requests')    
    grid.set_label('gridlines')
    others.set_label('stations')
    plt.legend(loc= 'upper right')
...
def combinedRequest(dayCount, source):
    ...
    #         print("\t\t Outpost", needs, "needs", request[req][needs])
...
def generatePath(sts, req, shipment, n, emReq):
    # for i in range(V):
    #     graph[i] = list(set(list(np.random.randint(0,V,np.random.randint(0,V)))))
    
    # inititalize_graph(sts, req, emReq)
    define_graph()
    ...
    for pi, label in enumerate(labels):
        seperatedPoints[label].append(points[pi])
    
    distances, pred = floydWarshall()
    for j in range(n):
        # seperatedPath, sepPred = getRoute(graph, src, seperatedPoints[j])
        seperatedPath, sepPred = sample8.generatPath(src, points, distances), pred
    ...
    print("\nGenerationally preferred path: ", sample8.generatPath(src, points, floydWarshall()[0]))
    print("\nGreedy Approach path:", getRoute(graph, src, points)[0])